...
sub _is_out_of_hours {
    ...
    @nwd or @nwd = (1,7);
    my $time = localtime;
    return 1 if $time->hour > $hrs{closed}{h} || ($time->hour == $hrs{closed}{h} && $time->min >= $hrs{closed}{m});
    return 2 if $time->hour < $hrs{open}{h} || ($time->hour == $hrs{open}{h} && $time->min >= $hrs{open}{m});
    return 3 if grep { $time->wday == $_ } @nwd;
    return 4 if $self->is_public_holiday() && not $ignore_public_hols;
    return 0;
}
